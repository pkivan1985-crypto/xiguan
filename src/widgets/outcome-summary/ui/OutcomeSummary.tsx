...
function OutcomeSummary({ batch, onBack }: OutcomeSummaryProps) {
	...
	return <main className={styles.summary}>
		...
		<section className={styles.list}><h3>{t('shell.today.summaryResults')}</h3>{batch.items.map((item) => <div className={styles.row} key={`${item.slotIndex}:${item.userCardId}`}><span className={styles.icon}><FiActivity /></span><div><strong>{item.cardTitle}</strong>{item.stageChange && <small>{t('shell.today.goalProgressLine', { goal: t('shell.today.stageChange'), before: progressValue(item, item.stageChange.before.quantityBaseValue), after: progressValue(item, item.stageChange.after.quantityBaseValue) })}</small>}{item.longTermChange && <small>{t('shell.today.goalProgressLine', { goal: t('shell.today.longTermChange'), before: progressValue(item, item.longTermChange.before.quantityBaseValue), after: progressValue(item, item.longTermChange.after.quantityBaseValue) })}</small>}{!item.longTermChange && !item.stageChange && <small>{t('shell.today.batchSaved')}</small>}</div><b>{formatOutcomeItem(item)} {item.displayUnit}</b></div>)}</section>
