...
interface CardDeckCopy {
	...
	bookkeepingEventDriven: string;
	bookkeepingEventDrivenHint: string;
	deleteAction: string;
...
function planSummary(card: DeckCardView, copy: CardDeckCopy): string {
	if (card.template.id === 'bookkeeping') return copy.bookkeepingEventDriven;
...
function todayReference(card: DeckCardView, copy: CardDeckCopy): string {
	...
	if (card.todayStatus.kind === 'event') return card.template.id === 'bookkeeping' ? copy.bookkeepingEventDriven : copy.eventDriven;
...
function ExpandedHabitCard({
	...
	return (
		...
		>
			...
			) : <p className={styles.noGoal}>{card.template.id === 'bookkeeping' ? copy.bookkeepingEventDrivenHint : card.template.id === 'extra-expense' ? copy.eventDrivenHint : copy.noGoal}</p>}
	...
	return (
		...
		>
			<button type='button' aria-expanded='false' onClick={onToggle}>
				...
				{percentage === null ? (
					<span className={styles.compactStatus}>{card.template.id === 'bookkeeping' ? copy.bookkeepingEventDrivenHint : card.template.id === 'extra-expense' ? copy.eventDrivenHint : copy.noGoal}</span>
				) : (
