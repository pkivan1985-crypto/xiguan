...
describe('saveTodayOutcome', () => {
	it('seeds approved definitions idempotently and rejects an empty draft without writes', async () => {
		...
		await expect(saveTodayOutcome(database, { localDate: LOCAL_DATE, nowIso: '2026-07-11T09:00:00.000Z', submissionId: 'submission-empty' })).rejects.toThrow('TODAY_DRAFT_EMPTY');
	...
	it('rejects date drift, invalid values, and unconfirmed values above 100 km', async () => {
		...
		await expect(saveTodayOutcome(database, { localDate: '2026-07-12', nowIso: '2026-07-11T09:00:00.000Z', submissionId: 'submission-date' })).rejects.toThrow('TODAY_DRAFT_NOT_FOUND');
		await expect(saveTodayOutcome(database, { localDate: LOCAL_DATE, nowIso: '2026-07-11T09:00:00.000Z', submissionId: 'submission-limit' })).rejects.toThrow('QUANTITY_CONFIRMATION_REQUIRED');
		await table('todayDrafts').update(LOCAL_DATE, { slots: makeDraft('1e2').slots });
		await expect(saveTodayOutcome(database, { localDate: LOCAL_DATE, nowIso: '2026-07-11T09:00:00.000Z', submissionId: 'submission-invalid' })).rejects.toThrow('INVALID_QUANTITY');
		expect(await table('actionRecords').count()).toBe(0);
	});

	it('returns the original batch when the same submission is retried', async () => {
		await arrangeDraft();
		const input = { localDate: LOCAL_DATE, nowIso: '2026-07-11T09:00:00.000Z', submissionId: 'submission-a' };
	...
	it('overwrites the same-day record instead of accumulating it', async () => {
		...
		await saveTodayOutcome(database, { localDate: LOCAL_DATE, nowIso: '2026-07-11T09:00:00.000Z', submissionId: 'submission-a' });
		await table('todayDrafts').update(LOCAL_DATE, { slots: makeDraft('7.5').slots });

		await saveTodayOutcome(database, { localDate: LOCAL_DATE, nowIso: '2026-07-11T10:00:00.000Z', submissionId: 'submission-b' });
	...
	it('completes active goals, records revisions, and freezes progress in the batch', async () => {
		...
		const batch = await saveTodayOutcome(database, { localDate: LOCAL_DATE, nowIso: '2026-07-11T09:00:00.000Z', submissionId: 'submission-goal' });
		...
		expect(batch.items[0]).toMatchObject({ longTermGoalId: 'long-a', stageGoalId: 'stage-a', longTermProgressRatio: 1, stageProgressRatio: 1 });
	});
	...
	it('does not count records associated with an older goal', async () => {
		...
		const batch = await saveTodayOutcome(database, { localDate: LOCAL_DATE, nowIso: '2026-07-11T09:00:00.000Z', submissionId: 'submission-new-goal' });

		expect(await table('longTermGoals').get('long-a')).toMatchObject({ status: 'active' });
		expect(batch.items[0]).toMatchObject({ longTermGoalId: 'long-a', longTermProgressRatio: 0.2 });
	});
	...
	it('rolls back records, goals, revisions, batch, and draft when the late batch write fails', async () => {
		...
		try {
			await expect(saveTodayOutcome(database, { localDate: LOCAL_DATE, nowIso: '2026-07-11T09:00:00.000Z', submissionId: 'submission-fail' })).rejects.toThrow('INJECTED_BATCH_FAILURE');
		} finally {
