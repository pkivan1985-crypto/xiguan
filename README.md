...
当前版本为 `3.0.0-rc.12` 公开候选：完成打卡后修改公里数、用时、配速、心率或备注不会再取消完成状态；跑步配速改为分钟与秒数分段输入，只需填写数字。今天页仍按跑步、喝水、阅读、早睡、少刷手机和轻食等习惯类型提供个性化记录内容，长期目标推导每日安排、可选多阶段推进、进展月历、习惯归档，以及普通/加密备份、原子恢复和由用户控制的 PWA 安装、离线提示及安全更新流程继续保留。
